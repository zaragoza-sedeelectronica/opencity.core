...
public class JPABaseDAO {
	...
	protected boolean _removeById(Class<?> type, Serializable id) {
		if (id != null) {
			...
			if (!query.getResultList().isEmpty()) {
				...
				this.getSolr().deleteById(type, id);
				return true;
			}
		}
		return false;
	...
	protected <T> T _merge(T entity) {
		...
		if (this.getSolr().indizar(entidad)) {
			this.getSolr().update(entidad);
			this.getSolr().commit();
		}
		return entidad;
	}
	 ...
	 * @see ISearch
	 */
	protected List<Object> _search(ISearch search) {
		if (search == null)
	...
	private boolean validId(Serializable id) {
		if (id == null 
				|| (id instanceof Number && ((Number) id).equals(0))
				|| (id instanceof String && "".equals(id))){
			return false;
		}
		return true;
	}
