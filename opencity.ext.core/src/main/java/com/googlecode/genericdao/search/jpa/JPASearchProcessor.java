...
public class JPASearchProcessor extends BaseSearchProcessor {
	...
	public List<Object> search(EntityManager entityManager, ISearch search) {
		if (search == null)
			return null;
	...
	public List<Object> search(EntityManager entityManager, Class<?> searchClass, ISearch search) {
		...
		if (search.getFields() != null && !search.getFields().isEmpty()) {
			List<Object> listado = transformResults(query.getResultList(), search);
			List<Object> retorno = new ArrayList<Object>();
			for (Object entity : listado) {
				if (entity instanceof Object[]) {
					Object[] objeto = (Object[]) entity;
					Iterator<Field> fieldItr = search.getFields().iterator();
					int i = 0;
					
					try {
					    Object t = searchClass.newInstance();
						while (fieldItr.hasNext()) {
							Field field = fieldItr.next();
							java.lang.reflect.Field campo = searchClass.getDeclaredField(field.getProperty());
							campo.setAccessible(true);
							campo.set(t, objeto[i]);
							i++;
						}
						retorno.add(t);
	...
	public SearchResult<?> searchAndCount(EntityManager entityManager, Class<?> searchClass, ISearch search) {
		...
		if (search.getMaxResults() > 0) {
			...
			result.setStart(search.getFirstResult());
			result.setRows(search.getMaxResults());
		} else {
