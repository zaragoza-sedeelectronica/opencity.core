...
public class JPAAnnotationMetadataUtil implements MetadataUtil {
	...
	public Metadata get(Class<?> rootEntityClass, String propertyPath) {
		if (rootEntityClass.isAnnotationPresent(Rel.class)) {
			HashMap<String, String> transientField;
			try {
				transientField = (HashMap<String,String>)rootEntityClass.getField("transientField").get(null);
				if (transientField.get(propertyPath) != null) {
					propertyPath = transientField.get(propertyPath);
				}
			} catch (Exception e) {
				;
			}
