...
public abstract class BaseSearchProcessor {
	...
	protected String generateSelectClause(SearchContext ctx, List<Field> fields, boolean distinct) {
		...
		if (fields != null) {
			for (Field field : fields) {
				...
				sb.append( " as " + prop.substring(prop.indexOf('.') + 1, prop.length()));
	...
	protected String generateOrderByClause(SearchContext ctx, List<Sort> sorts) {
		...
		for (Sort sort : sorts) {
			...
			sb.append(sort.isDesc() ? " desc NULLS LAST" : " asc NULLS LAST");
		}
		if (first) {
			return "";
		}
	...
	protected String filterToQL(SearchContext ctx, Filter filter) {
		...
		try {
			if (ctx.rootClass.isAnnotationPresent(Rel.class)) {
				HashMap<String,String> transientField = (HashMap<String,String>)ctx.rootClass.getField("transientField").get(null);
		...
		case Filter.OP_DISTANCIA:
			
			double[] valor = (double[]) value;
//			return "x is not null and SDO_GEOM.SDO_DISTANCE(MDSYS.SDO_GEOMETRY(2001,NULL,"
...
//					+ "< " + valor[0];
		...
		case Filter.OP_EQUAL:
			try {
				java.lang.reflect.Field nombreCampo = ctx.getRootClass().getDeclaredField(property);
				if (value instanceof Date
			        ...
			        return getPathRef(ctx, property) + " >= " + param(ctx, value) + " and " + getPathRef(ctx, property) + " <= " + param(ctx, cal.getTime());
				} 
			} catch (Exception e) {
		...
		case Filter.OP_NONE:
			if (!(value instanceof Filter)) {
				...
				// up working as desired. FIXME: If and when the Hibernate bug is
...
//			MODIFICADO 1 expectedClass = metadataUtil.get(rootClass, "id").getJavaClass();
//			MODIFICADO 2 
			expectedClass = metadataUtil.get(rootClass, property).getJavaClass();
//			if (property.indexOf(".") > 0) {
//				expectedClass = metadataUtil.get(rootClass, property.substring(property.lastIndexOf(".") + 1, property.length())).getJavaClass();
//			} else {
//				expectedClass = metadataUtil.get(rootClass, property).getJavaClass();	
//			}
//			--MODIFICADO
	...
	public static final String SORTDISTANCIA = "ordenarPorDistancia";
	...
	private void getFilterFromExampleRecursive(Object example, Metadata metadata, ExampleOptions options,
		...
		for (String property : metadata.getProperties()) {
			if (options.getExcludeProps() != null && !options.getExcludeProps().isEmpty()) {
