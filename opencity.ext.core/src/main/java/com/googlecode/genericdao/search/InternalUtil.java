...
public class InternalUtil {
	...
	public static Object convertIfNeeded(Object value, Class<?> type) {
		...
		if (String.class.equals(type)) {
			...
			try {
				return ConvertDate.string2Date(value.toString(), ConvertDate.DATEQUERY, Locale.US);
			} catch (ParseException e) {
				throw new ClassCastException("Unable to convert value " + value.toString() + " to type "
						+ type.getName());
			}
